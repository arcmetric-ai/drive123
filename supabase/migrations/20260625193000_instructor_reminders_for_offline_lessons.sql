create or replace function private.external_learner_display_name(target_id uuid)
returns text
language sql
stable
security definer
...
as $$
  select coalesce(
    nullif(trim(external.full_name), ''),
    'your offline learner'
  )
  from public.external_learners external
  where external.id = target_id;
$$;

revoke all on function private.external_learner_display_name(uuid) from public;
revoke all on function private.external_learner_display_name(uuid) from anon;
revoke all on function private.external_learner_display_name(uuid) from authenticated;
...
declare
  learner_name text := coalesce(
    private.profile_display_name(new.learner_id),
    private.external_learner_display_name(new.external_learner_id),
    'your learner'
  );
...
begin
  if new.learner_id is null and new.external_learner_id is null then
    return new;
  end if;
  ...
  if tg_op = 'INSERT' then
    ...
    if new.learner_id is not null and (
    ...
    if new.learner_id is not null and new.status is distinct from old.status then
